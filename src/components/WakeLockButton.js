...
const WakeLockButton = ({ className = "" }) => {
  ...
  // No mostrar el botón si no es compatible o si está cargando
  ...
  const tooltipText = isLoading 
    ...
    : (isVisuallyActive 
      ? `Desactivar pantalla siempre encendida${preferences.dimScreenOnWakeLock ? ` (Brillo: ${Math.round(currentBrightness * 100)}%)` : ''}${!isWakeLockActive && userIntendedActive ? ' (Activando...)' : ''}`
  ...
  return (
    ...
    >
      ...
      <span className="sm:hidden">
        {isLoading ? '...' : (isVisuallyActive ? 'ON' : 'OFF')}
      </span>
    </button>
  );
};
